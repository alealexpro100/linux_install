...
if [[ $WORK_MODE == "install" ]]; then
    ...
    while [[ -z $PART_BOOT || -z "$PART_ROOT" ]]; do
        ...
        if [[ $PART_DO == "1" ]]; then
            read_param "" "$M_PART_D" "" PART_ROOT text_check "$(lsblk -nrd -o NAME | sed -e '/loop[0-10]/d' | tr '\n' ',' | sed -e 's/,$//')"
            cfdisk -z "/dev/$PART_ROOT"
        else
            read_param "$M_PART_I_M\n" "$M_PART_P" "" PART_ROOT text_check "$(lsblk -nr -o NAME | sed -e '/loop[0-10]/d' | tr '\n' ',' | sed -e 's/,$//')"
            if [[ -d /sys/firmware/efi/efivars ]]; then
                BOOTLOADER_TYPE_DEFAULT=bios
                msg_print note "$M_BOOTLOADER_TYPE: $BOOTLOADER_TYPE_DEFAULT."
                read_param "$M_PART_B_M\n" "$M_PART_P" "" PART_BOOT text_check "$(lsblk -nr -o NAME | sed -e '/loop[0-10]/d' | tr '\n' ',' | sed -e 's/,$//')"
                [[ $(findmnt -Recvruno FSTYPE "$PART_BOOT") != "vfat" ]] && msg_print warning "Partition $PART_BOOT is NOT a vfat filesystem." 
            else
                BOOTLOADER_TYPE_DEFAULT=bios
                msg_print note "$M_BOOTLOADER_TYPE: $BOOTLOADER_TYPE_DEFAULT."
                read_param "" "$M_BOOTLOADER_PATH" "" PART_BOOT text_check "$(lsblk -nr -o NAME | sed -e '/loop[0-10]/d' | tr '\n' ',' | sed -e 's/,$//')"
            fi
