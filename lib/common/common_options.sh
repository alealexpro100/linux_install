...
if [[ -z $passwd ]]; then
  ...
  print_param warning "No password entered. Password set to $passwd."
fi
if mountpoint -q "$dir" && [[ $(findmnt -funcevo SOURCE $dir) != tmpfs ]]; then
  read_param "" "Do you want to generate fstab?" '' fstab yes_or_no
  read_param "" "Do you want to install bootloader?" '' bootloader yes_or_no
  if [[ $bootloader == "1" ]]; then
    ...
    read_param "" "Do you want to install $distr to removable disk?" '' removable_disk no_or_yes
  fi
else
  bootloader=0
fi
...
read_param "" "Do you want to copy this setup utitlity to new OS?" '' copy_setup_script yes_or_no
